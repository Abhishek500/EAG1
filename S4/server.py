...
# We hold a global reference to the paint application:
paint_app = None
...
async def draw_rectangle(x1: int, y1: int, x2: int, y2: int) -> dict:
    ...
    IMPORTANT: We handle the coordinate system carefully:
      - Use absolute coords for clicking the rectangle tool.
      - For the actual drawing, we pass absolute coordinates so that the press-and-drag is recognized.
    """
    global paint_app
    try:
        if not paint_app:
        ...
        paint_window.click_input(coords=(657, 106), absolute=True)
